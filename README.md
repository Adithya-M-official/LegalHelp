...
LegalHelp is a Streamlit application that helps everyday people understand legal documents in plain language. A user uploads one or more photos of a legal document (single- or multi-page) and asks a question — either by typing or recording their voice — and LegalHelp uses Google's Gemini model to read the document, check the images for common quality problems, answer the question in simple terms, and speak the answer back in the same language it was asked in.

## Problem Statement

Legal documents are often dense, jargon-heavy, and inaccessible to people without legal training. This creates a real barrier for individuals with limited literacy, limited time, or limited familiarity with legal language — who may end up signing, ignoring, or misunderstanding documents that materially affect their lives. LegalHelp aims to lower that barrier by turning a photo (or set of photos) and a spoken or typed question into a clear, plain-language explanation.

## Features

- 📷 **Image-based document input** — upload one or more photos of a legal document (PNG/JPG/JPEG).
- 📄 **Multi-page support** — upload several page images at once (up to 10) and LegalHelp reads them together as a single document, in the order uploaded.
...
- ✅ **Upfront input validation** — per-page file size limits, image-format checks, page-count limits, and question-length limits catch problems before an API call is made.
...
1. **UI layer (`app.py`)** collects one or more page images and a question from the user via Streamlit, sets up application-wide logging, and renders quality warnings and results.
2. **Page validation (`document_pages.py`)** validates each uploaded page (size, format) and runs it through the image quality checks.
3. **Image quality checks (`image_quality.py`)** screen each page for blur, glare, low resolution, and poor lighting using lightweight Pillow-based heuristics — no extra image-processing dependency required.
4. **Logic layer (`ai_logic.py`)** builds a multimodal request (one or more page images + text or audio), sends it to the Gemini API along with the system prompt, and robustly parses the structured JSON response.
5. **Response handling** — the parsed explanation text is converted to speech with gTTS, with language-code resolution to avoid unnecessary fallbacks.
6. **Export (`export.py`)** — builds an optional PDF version of the explanation for download.
7. **Output** — the text explanation, generated audio, and download options are displayed back in the Streamlit UI.
...
```
User uploads 1+ page images + asks question (text or audio)
            │
            ▼
        app.py (Streamlit UI)
            │
...
1. Upload one or more photos of a legal document (PNG/JPG/JPEG). For multi-page documents, upload all pages at once, in reading order (up to 10 pages).
...
3. Click **Analyze**. If any page has a quality issue (blur, glare, low resolution, poor lighting), you'll see a warning before the explanation is generated.
...
├── document_pages.py               # Per-page upload validation and quality-check orchestration
...
- Responses depend entirely on Gemini's ability to read the uploaded image(s) accurately. LegalHelp now screens for common problems (blur, glare, low resolution, poor lighting) and warns the user beforehand, but this is a heuristic screen, not a guarantee — a technically "sharp" photo can still be misread if handwriting is illegible or the document itself is degraded.
...
Implemented in this update: multi-page document uploads, and PDF export of the explanation.
